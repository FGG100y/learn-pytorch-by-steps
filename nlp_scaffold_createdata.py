...
def main(verbose=2):
    ...
    f1name = Path(
        parse.urlsplit(nlpu.ALICE_URL).path
    ).name  # "alice28-1476.txt"
    f2name = Path(
        parse.urlsplit(nlpu.WIZARD_URL).path
    ).name  # "wizoz10-1740.txt"

    try:
        alice_text = load_source_text(
            fname=f1name, fileconfig=config, skip_header=True
        )
        wizoz_text = load_source_text(
            fname=f2name, fileconfig=config, skip_header=True
        )
    ...
    if verbose:
        print(
            "Num. of sentences in texts:", len(corpus_alice), len(corpus_wizoz)
        )
        if verbose == 2:
    ...
    if len(new_fnames) == 0:  # if no such files, create them
        ...
        for fname in (f1name, f2name):
            new_fname = sentence_tokenize(fname=fname, fileconfig=config)
...
def load_source_text(
    fname, fileconfig, localfolder="texts", skip_header=False
):
...
def sentence_tokenize(
    ...
    **kwargs,
    ...
    source = load_source_text(fname, fileconfig=fileconfig, skip_header=True)
